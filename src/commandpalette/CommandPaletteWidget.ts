...
import type { IParseTreeNode, IWidgetEvent, IWidgetInitialiseOptions } from 'tiddlywiki';
...
class CommandPaletteWidget extends Widget {
  ...
  actionStringBuilder(text: string) {
    return (e: AllPossibleEvent) => this.invokeActionString(text, this, e);
  }

  actionStringInput(action: string, hint: string, _e: AllPossibleEvent) {
      ...
      },
      {
        ...
        action: (e) => {
          this.tagOperation(
            ...
            '选择一个条目来去除标签',
            '选择一个标签来去除 (⇧⏎ 可以去除多次)',
            (tiddler: string, terms: string): string[] => $tw.wiki.filterTiddlers(`[[${tiddler}]tags[]] +[${$tw.utils.pinyinfuse ? 'pinyinfuse' : 'search'}[${terms}]]`),
            false,
            'tm-remove-tag',
          );
        },
    ...
    for (const tiddler of commandTiddlers) {
      ...
      if (type === 'prompt') {
        ...
        const action = {
          ...
          hint,
          action: () => {
            this.promptCommand(textFirstLine, caret);
          },
      ...
      if (type === 'prompt-basic') {
        ...
        const action = {
          ...
          hint,
          action: () => {
            this.promptCommandBasic(textFirstLine, caret, hint);
          },
          keepPalette: true,
        };
        this.actions.push(action);
      ...
      if (type === 'message') {
        this.actions.push({
          ...
          hint,
          action: (e: AllPossibleEvent) => {
            this.tmMessageBuilder(textFirstLine)(e);
          },
          keepPalette: false,
        });
        continue;
      ...
      if (type === 'actionString') {
        ...
        if (userInput) {
          this.actions.push({
            ...
            hint,
            action: (e: AllPossibleEvent) => {
              this.actionStringInput(text, hint, e);
            },
            keepPalette: true,
          });
        } else {
  ...
  explorerProvider(url: string, _terms: string) {
    const switchFolder = (url: string) => {
      ...
      this.currentProvider = (terms: string) => {
        this.explorerProvider(url, terms);
      };
      this.onInput();
    };
  ...
  render(parent, nextSibling) {
    ...
    $tw.rootWidget.addEventListener('open-command-palette-selection', (e: AllPossibleEvent) => {
      this.openPaletteSelection(e);
    });

    $tw.rootWidget.addEventListener('insert-command-palette-result', (e: AllPossibleEvent) => {
      this.insertSelectedResult();
    });

    $tw.rootWidget.addEventListener('command-palette-switch-history', (e) => {
      this.handleSwitchHistory(e, true);
    });

    $tw.rootWidget.addEventListener('command-palette-switch-history-back', (e) => {
      this.handleSwitchHistory(e, false);
    });
    ...
    document.addEventListener('click', (e: PointerEvent | MouseEvent | TouchEvent) => {
      this.onClick(e);
    });
    ...
    this.symbolProviders['>'] = {
      searcher: (terms: string) => {
        this.actionProvider(terms);
      },
      resolver: (e) => {
        this.actionResolver(e);
      },
    };
    this.symbolProviders['》'] = this.symbolProviders['>'];
    this.symbolProviders['##'] = {
      searcher: (terms: string) => {
        this.tagListProvider(terms);
      },
      resolver: (e) => {
        this.tagListResolver(e);
      },
    };
    this.symbolProviders['#'] = {
      searcher: (terms: string) => {
        this.tagProvider(terms);
      },
      resolver: (e) => {
    ...
    this.symbolProviders['？'] = this.symbolProviders['?'];
    this.symbolProviders['['] = {
      searcher: (terms: string, hint?: string) => {
        this.filterProvider(terms, hint);
      },
      resolver: (e) => {
        this.filterResolver(e);
      },
    };
    this.symbolProviders['+'] = {
      searcher: (terms: string) => {
        this.createTiddlerProvider(terms);
      },
      resolver: (e) => {
    ...
    this.symbolProviders['|'] = {
      searcher: (terms: string) => {
        this.settingsProvider(terms);
      },
      resolver: (e) => {
        this.settingsResolver(e);
      },
    };
    this.currentResults = [];
    this.currentProvider = () => {};
  ...
  addResult(result: IResult, id: number) {
    ...
    if (result.action != undefined) {
      resultDiv.addEventListener('abort', result.action as unknown as (this: GlobalEventHandlers, event_: UIEvent) => any);
    }
  ...
  parseTags(input: string) {
    ...
    let tagsFilter = `[all[tiddlers+system+shadows]${
      tags.reduce((a, c) => {
        return a + 'tag[' + c + ']';
      }, '')
    }]`;
  ...
  settingsProvider(_terms: string) {
    ...
    this.showResults([
      {
        name: 'Theme (currently ' + this.settings.theme?.match?.(/[^/]*$/) ?? 'no ' + ')',
        action: () => {
          this.promptForThemeSetting();
        },
      },
  ...
  settingResultBuilder<K extends keyof ISettings>(name: string, settingName: K, hint: string, validator: IValidator, errorMessage: string) {
    return {
      ...
      action: () => {
        this.promptForSetting(settingName, hint, validator, errorMessage);
      },
    };
  }
